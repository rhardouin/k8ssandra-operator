...
func TestNormalizeManagerImageID(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "digest only falls back to configured reference", imageID: "docker://" + digest, want: "registry.example/operator:v1"},
		...
		{name: "digest only rejects blank configured image", imageID: "docker://" + digest, configuredImage: " ", wantErr: true},
